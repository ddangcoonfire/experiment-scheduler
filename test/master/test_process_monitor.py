...
class MockTask:
    def __init__(self):
        self.tasks = {
            'RUN': {'task_id': '1', 'task_manager': 'tm_address1', 'status': task_manager_pb2.TaskStatus.RUNNING},
            'DONE': {'task_id': '2', 'task_manager': 'tm_address1', 'status': task_manager_pb2.TaskStatus.DONE},
            'KILL': {'task_id': '3', 'task_manager': 'tm_address1', 'status': task_manager_pb2.TaskStatus.KILLED},
            'ABNORMAL': {'task_id': '4', 'task_manager': 'tm_address1', 'status': task_manager_pb2.TaskStatus.ABNORMAL}}
...
class TestProcessMonitor(TestCase):
    ...
    def setUp(self):
        ...
        self.process_monitor.task_manager_stubs = {'tm_address1': MockTaskManagerStub(),
    ...
    def test_run_task(self):
        ...
        # when
        test_return_value = self.process_monitor.run_task(run_task.task_id, run_task.task_manager, run_task.command,
    ...
    def test_kill_task(self):
        ...
        # when
        test_return_value = self.process_monitor.kill_task(task['task_manager'], str(task['task_id']))
    ...
    def test_get_task_status(self):
        ...
        for task in self.mock_task.tasks.values():
            # when
            test_return_value = self.process_monitor.get_task_status(task['task_manager'], str(task['task_id']))
    ...
    def test_get_all_tasks(self):
        ...
        for address in self.process_monitor.task_manager_address:
            ...
            for task in self.mock_task.tasks.values():
                all_task_status.append(task_manager_pb2.TaskStatus(task_id=task['task_id'],
                                                                                     status=task['status']))
